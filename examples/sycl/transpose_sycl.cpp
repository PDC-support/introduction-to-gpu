...
// Simple kernel for transposing a matrix
auto transposeKernelSimple(sycl::handler & /*cgh*/, const float *in, float *out,
...
// Trasnposing a matrix using local cache: better global memory access patterns
auto transposeKernelLocal(sycl::handler &cgh, const float *in, float *out,
                          int width, int height) {
  ...
  return [=](sycl::nd_item<2> item) {
    ...
    // Preload data into the buffer
    tile[y_local_index * tile_dim + x_local_index] = in[in_index];
    // Synchronize withing group
    item.barrier(); // Like __syncthreads()
    out[out_index] = tile[x_local_index * tile_dim + y_local_index];
  };
}

// Change which kernel flavor to use
const auto transposeKernel = transposeKernelSimple;
...
int main() {
  ...
  // TODO: Allocate output array d_out on the device (same size)

  // TODO: Copy the data from matrix_in.data() to d_in
